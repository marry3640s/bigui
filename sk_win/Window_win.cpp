    ...
    switch (message) {
		...
		case WM_RBUTTONUP: {
			...
			if (message == WM_RBUTTONDOWN)
				istate = skui::InputState::kDown;
			else if(message == WM_RBUTTONUP)
				istate = skui::InputState::kUp;
			eventHandled = window->onRMouse(xPos, yPos, istate, skui::ModifierKey::kNone);
		...
		case WM_IME_NOTIFY:
			window->onIMEMessage(message, wParam, lParam);
			break;
		...
		case WM_MOUSEWHEEL: {
			int xPos = GET_X_LPARAM(lParam);
			int yPos = GET_Y_LPARAM(lParam);
			eventHandled = window->onMouseWheel(xPos, yPos, GET_WHEEL_DELTA_WPARAM(wParam) > 0 ? +1.0f : -1.0f,
				get_modifiers(message, wParam, lParam));
			break;
		}
