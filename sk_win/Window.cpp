...
bool Window::onRMouse(int x, int y, skui::InputState state, skui::ModifierKey modifiers) {
	return this->signalLayers([=](Layer* layer) { return layer->onRMouse(x, y, state, modifiers); });
}

int Window::onIMEMessage(unsigned int nMsg, unsigned int wparam, int lparam) {
	return this->signalLayers([=](Layer* layer) { return layer->onIMEMessage(nMsg,wparam,lparam); });
}

bool Window::onMouseWheel(int x, int y,float delta, skui::ModifierKey modifiers) {
    return this->signalLayers([=](Layer* layer) { return layer->onMouseWheel(x,y,delta, modifiers); });
}
